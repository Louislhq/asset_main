    ...
    var SelectArea = function(options) {
        ...
        var g = this,
            domFragment = {
                ...
                search: '<div class="l-selectArea-search ui-form"><label><input type="text" class="ui-input" placeholder="请输入关键字"  /></label><span class="ui-btn">搜索</span></div>',
            ...
            c = {
                fnClose: function() {
                    ...
                    closeBtn
                        .on('click', function() {
                            selectAreaMain.hide();
                            if (!g.isOtherSence) {
                                selectAreaMain.find('.l-selectArea-sceneryMain').remove();
                                selectAreaMain.find('input').val('');
                            }
                            openBtn.show();
                        });
                    openBtn
                ...
                _each: function(tpl, data) {
                    ...
                    len = !g.isOtherSence && len > 20 ? 20 : len;
                    for (var i = 0; i < len; i++) {
                        data[i].index = i;
                ...
                init: function() {
                    ...
                    if (g.type === 1) {
                        ...
                        show = g.isOtherSence ? '' : 'style="display:none;"';
                        countyHtml = '<select class="l-selectArea-county ui-select" data-index="0"' + show + '>' + countyHtml + '</select>';
                    ...
                    sceneryHtml = g.isOtherSence ? sceneryHtml : '';
                    ...
                    html += '<div class="l-selectArea-selectedWrap">' +
                        ...
                        domFragment.search +
                        '</div>' +
                        '<div class="col-red1">注意：请选择发票上的酒店名称，如无法搜索到，请联系技术支持：0592-5372105</div>' +
                        sceneryHtml +
                        '</div>' +
                        '<div class="l-selectAreaBottom"><span class="l-selectArea-close ui-btn">确定</span></div>' +
                        '</div>';
