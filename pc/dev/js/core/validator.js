define(['core/basics'], function(basics){
	...
	*/
	...
	var Validator = function(o){
			...
			c = {
                ...
                rule: {
                    ...
                    phone: function(sVal, oItem){
                        ...
                        if( !/(1[345789]{1}\d{9}$)|(0\d{2,3}-\d{7,8}(-\d{2,3})?$)/.test(sVal) ){
				...
				route: function(oItem){
					...
					var oThat      = this,
						...
						rFormat    = /format\=|ajax\=/,
						rPosition  = /\|/,
						isFunction = basics.base.isFunction;
				...
				handleMessage: function(oSelf, sContents, sType, sTypeVal, position){
					var oThat     = this,
						...
						message   = parents.first().find('.ui-form-message'),
				...
				run: function(){
					var oThat      = this,
						...
						fAction    = function(oSelf){
                                        ...
                                        if( basics.base.isFunction(fRules) && sRule === 'process' ){
                                            return processHandle(sRule, fRules(oSelf), true );
                                        }

                                        if( basics.base.isFunction(fAjax) && sRule === 'ajax' ){
                                            processHandle(sRule);
		...
		g.reset = function(self){
			var oTarget  = self || p.target,
