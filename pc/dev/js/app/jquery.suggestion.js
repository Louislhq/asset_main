(function (root, factory) {
    ...
    var defaults      = null,
        suggestion    = function(defaults, self, selfIndex){
            ...
            var parent = '';
            //修复重新定义会叠加的问题
            if(self.parent().hasClass('l-suggestion')){
                self.parent().find("input:hidden").remove();
                self.parent().find('.l-suggestion-list').remove();
                parent = self.parent();
            }else{
                parent = self.wrap('<div class="l-suggestion fn-left"></div>').parent();
            }
            parent.append('<input type="hidden" name="'+ name +'" value="'+ value +'" /><ul class="l-suggestion-list" data-index="0" style="display:none"></ul>');
            var listWrap       = parent.find('.l-suggestion-list'),
                ...
                createList     = function(data){
                    ...
                    for(var i = 0, len = data.length; i < len && i < 20; i++){
                        item       = data[i];
            ...
            listWrap
                ...
                .on('click', 'li', function(e){
                    ...
                    if (! defaults.autoClose) {
                        console.log(defaults.autoClose);
